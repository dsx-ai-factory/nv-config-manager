...
1. For certificate delivery, the PKI provider and device certificate context
   are configured; Cumulus retrieves the material through the ZTP SFTP proxy
...
- **ZTP service traffic**: Devices must be able to access the ZTP server on TCP 80 for bootstrap and TCP 2222 for sensitive configuration and certificate delivery. TCP 443 is optional.
...
Cumulus certificate delivery uses SFTP on TCP 2222, so it does not require DNS,
an HTTPS IP SAN, or preloaded trust for the ZTP TLS certificate. See
...
Most device endpoints accept either a registered device request or an authenticated user request:
...
- The certificate endpoint accepts only a registered device request; user authentication does not bypass its source-IP check
...
- **426 Upgrade Required**: Use the dedicated ZTP LoadBalancer HTTPS endpoint for identity certificates
- **502 Bad Gateway**: Check that the PKI provider returned a valid certificate, private key, and CA chain
- **503 Service Unavailable**: Check PKI source configuration, JWT authentication, and Vault connectivity
...
1. **Protect private material**: Use the encrypted ZTP SFTP certificate path; the optional HTTP identity endpoint requires HTTPS
1. **Limit bootstrap HTTP**: Restrict HTTP to the private provisioning network and use it only for bootstrap artifacts, including a public ZTP CA when required
...
1. **Monitor Provisioning**: Correlate the ZTP `http-lb` and `sftp` container logs during provisioning; the default flow uses both transports
