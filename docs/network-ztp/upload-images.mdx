...
1. Obtain the SHA256 published by the image's download source, such as `enterprise.nvidia.com`.
1. Compute the copied file's SHA256 and compare it with the published value:

   ```bash
   sha256sum {nfs-root}/{platform}/{version}/{filename}
   ```

   Do not update the manifest unless the values match. If they differ, remove
   the copied file and repeat the download or copy.
1. Edit `manifest.json` at the PVC root. If an entry already has the same
   `platform` and `version`, replace that entry. When the filename changes, do
   not append a second entry; remove the existing entry before adding the new
   one. Use the verified, vendor-published SHA256:
...
Use this approach only when the API is unavailable. The API and installer keep
the manifest consistent and avoid errors from manual edits.
...
Confirm the checksum reported by the server matches the verified,
vendor-published checksum recorded in the manifest:

```bash
curl -s "https://ztp.example.com/v1/files/cumulus-linux/5.14.0/cumulus-linux-5.14.0-mlx-amd64.bin/checksum" | jq -r .checksum
```

This endpoint returns the checksum from `manifest.json`; it does not hash the
stored file. The local checksum comparison in the manual copy procedure is
therefore required.
