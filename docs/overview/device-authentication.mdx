...
1. The device site comes from normalized data supplied by the selected DCIM provider.
...
1. Update the provider's credential mapping to select revision `r2` for the target username. With the bundled Nautobot provider, update the applicable `password_mappings` config context from `rotation: r1` to `rotation: r2`.
...
If render output does not include the expected account, check the provider's credential mapping and confirm the mapped key exists at the site's Vault/OpenBao path. With the bundled Nautobot provider, inspect the device's `password_mappings` config context.

If a workflow cannot connect, confirm External Secrets Operator synced the runtime secret, `NV_CONFIG_MANAGER_CONFIG_SECRET_PATH` is mounted in the Temporal worker pod, the `[site.<slug>]` section exists, and the normalized device-site slug from the selected provider matches the section name.

If a password rotation reports no diff, the intended configuration probably
still points at the old secret revision. Confirm that the new Vault/OpenBao key
exists, the selected provider's credential mapping selects the new revision,
and render has regenerated the intended configuration before re-running the
rotation workflow. With the bundled Nautobot provider, confirm that the
applicable `password_mappings` config context has the expected `rotation` value.
