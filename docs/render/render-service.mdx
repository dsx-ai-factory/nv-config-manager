...
The **DCIM event consumer** responds to provider-neutral change events for
objects such as devices, interfaces, cables, and IP addresses. The consumer
validates the event envelope, dispatches it to model-specific handlers, and
queues device renders. The built-in Nautobot provider translates its legacy
changelog payload at the provider boundary during the compatibility window.

The **device change consumer** responds to queued render requests from event
handlers. It executes renders with distributed locking and updates the Config
Store.

### External NATS administration

External NATS deployments using password or JWT authentication must provide a `tls://` or `wss://` endpoint. Secure WebSockets (`wss://`) establish TLS before the NATS handshake and do not require the native NATS `handshake_first` setting. Native `tls://` endpoints must enable TLS-first negotiation on the server. Add `handshake_first: true` to the server's existing `tls` block, retaining the deployment's certificate and verification settings:
...
For native `tls://` endpoints, the password- or JWT-authenticated client performs TLS before reading the initial server `INFO` message and does not fall back to INFO-first negotiation. NATS defaults to sending `INFO` before TLS, so a server without this setting can cause a TLS error or connection timeout even when its certificates and credentials are valid. The Helm chart does not configure an externally managed server; coordinate this setting with its administrator before connecting.

TLS-first avoids relying on an unauthenticated `INFO` message to request encryption before sending credentials. This requirement applies to external password and JWT authentication; the bundled `nats://` deployment retains its existing negotiation behavior.
...
**Template updater**: Runs as a Kubernetes job after Helm installs and upgrades.
It queries the selected DCIM provider for devices with a stale `template_version`
and renders those devices directly with bounded concurrency. Each update uses the
device's Redis-backed render lock.

**Version tracking**: A successful render records the
`nv_config_manager_templates` version through the selected DCIM provider. The
next template-updater job skips devices that already have the current version.

## Deployment

The service uses two consumer deployments (`nautobot` and `device`), a separate
template-updater job that runs after Helm installs and upgrades, and Redis for
distributed locking. It exposes Prometheus metrics on port 8000.
...
- **NAK**: Transient failures and lock acquisition failures (5s delay)
...
**Pull-based consumption**: Consumers fetch messages on demand instead of using
push-based subscriptions, which improves flow control and horizontal scaling.

**Distributed locking**: Redis-backed locks prevent concurrent renders for the same device across multiple consumer instances.

**Version-aware processing**: The template-updater job compares the current
template package version with the version recorded by the selected DCIM
provider. It renders stale devices directly with bounded concurrency and
per-device locks.
