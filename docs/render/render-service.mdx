...
**Version tracking**: The producer records the `nv_config_manager_templates`
version through the DCIM provider. The template consumer rejects events for
newer versions with a negative acknowledgement and a 30-second delay. During a
rolling deployment, old pods can terminate before new pods process the backlog.

## Deployment

The service uses three consumer deployments (DCIM, device, and template), a
producer job that runs on Helm upgrades, and Redis for distributed locking. It
...
**Version-aware processing**: The template consumer compares its running
version with the message version and rejects messages from newer versions
