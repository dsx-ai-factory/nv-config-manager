...
1. View pool details including address ranges:
   ...
   The output of the second command resembles the following example, which shows
   a pool of available IP addresses from `10.0.100.200` through
   `10.0.100.210`:
...
1. From the output of the second command, select two unused IP addresses for ZTP and DHCP services.
   ...
   Use these addresses with the `--ztp-lb-ip` and `--dhcp-lb-ip` flags when
   deploying Config Manager.
...
To verify that DHCP has sufficient data from the selected DCIM provider, check the logs of the `config-refresh-v4` container in the DHCP refresh pods.
...
You do not need to redeploy from scratch when you change data, add images, update templates, or upgrade versions. Treat `install.yaml` as the source of truth: re-open it with `./installer/nvcm-installer init --config install.yaml`, adjust the configuration, and deploy again.
