...
| Python 3.13+ and `uv` | Runs or installs the `nvcm-installer` CLI |
...
Devices enter Config Manager through the selected DCIM provider. With the bundled Nautobot provider, you can create them manually, through the Nautobot API, or with a post-deploy job such as a Nautobot Design Builder topology loader. The local SuperPOD profile uses the bundled mock topology Design Builder job in `development/mock_topology` as an example. Refer to [Design Builder Data Loading](../nautobot/design-builder-data-loading.mdx) for the job structure and the context-only customization path.

### How ZTP Starts

**ZTP is initiated by the device**, not by Config Manager. As soon as Config Manager has sufficient data from the selected DCIM provider, it serves DHCP on the configured network. When a device is cabled and powered on, it gets an IP address and boot file from DHCP, then retrieves its configuration and firmware from the ZTP service. The provider records successful ZTP completion; the bundled Nautobot provider sets the device status to `Provisioned`.
...
kubectl logs -n <namespace> -l component=consumer-device -c consumer -f --tail=500

# DCIM event consumer dispatches provider events.
kubectl logs -n <namespace> -l component=consumer-nautobot -c consumer -f --tail=500
```
