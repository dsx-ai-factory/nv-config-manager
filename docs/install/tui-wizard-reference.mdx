...
Sites represent data centers managed by this Config Manager deployment. Each site name must match the slug form of the site name returned by the selected DCIM provider; for the bundled Nautobot provider, use the corresponding Location slug. Sites scope per-site network secrets such as device login credentials and BGP passwords.
