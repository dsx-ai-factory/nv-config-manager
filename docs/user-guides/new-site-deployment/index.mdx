...
1. Open the **ZTP logs** panel and switch to the log explorer before filtering by device UUID. Include both the `http-lb` and `sftp` containers: provisioning starts over HTTP/HTTPS, then the default Cumulus template retrieves rendered configuration over SFTP.
...
| ZTP serves the device | The `http-lb` or `sftp` ZTP container logs contain the Nautobot device UUID | Correlate both containers. An HTTP status diagnoses bootstrap requests; an SFTP path or transfer error diagnoses rendered configuration retrieval. |
| Provisioning completes | The `http-lb` logs show the device calling the `provisioned` endpoint and Nautobot shows **Provisioned** | ZTP completed. If the intended config is still wrong, inspect the `sftp` and render-service logs and the current rendered configuration. |
...
Filter both device-facing ZTP containers by the Nautobot device UUID. The
`http-lb` container handles the boot script and HTTP/HTTPS callbacks; the
`sftp` container handles the default rendered-configuration download. A
complete provisioning trace can cross both containers.

```text
{cluster="<cluster>", k8s_namespace_name="<config-manager-namespace>", k8s_container_name=~"http-lb|sftp"} |= `<device-uuid>`
```
...
```bash
kubectl -n <config-manager-namespace> logs \
  -l app.kubernetes.io/component=network-ztp -c http-lb \
  --tail=-1 | grep '<device-uuid>'

kubectl -n <config-manager-namespace> logs \
  -l app.kubernetes.io/component=network-ztp -c sftp \
  --tail=-1 | grep '<device-uuid>'
```
...
```text
{cluster="<cluster>", k8s_namespace_name="<config-manager-namespace>", k8s_container_name=~"api|consumer|template-updater"} | json category | category=~"render.*" |= `<device-uuid>`
```
...
| Boot script starts, then provisioning stalls | SFTP reachability on TCP 2222 and the ZTP `sftp` container logs; also check `http-lb` for the preceding bootstrap request and later callback |
| Recent NOS provisioning failure | The final command and transport error in the vendor-specific provisioning log, then both the `http-lb` and `sftp` ZTP logs by device UUID |
...
The rendered configuration either failed to transfer over SFTP, failed to
apply, or broke connectivity back to Config Manager. Access the switch via
[console or upstream SSH](#options-for-logging-into-a-stuck-device), inspect
`/var/log/autoprovision`, and correlate the device UUID with the ZTP `sftp`
container logs.

**Cumulus Linux: console reports that ZTP is in progress for hours**

The banner or `nv show system ztp` state says that ZTP is enabled or incomplete; it does not prove that attempts are still occurring. Check the newest timestamp in `/var/log/autoprovision` and both the `http-lb` and `sftp` ZTP container logs for the device UUID.
...
- Confirm the configured HTTP/HTTPS bootstrap port and TCP 2222 for SFTP are open from the device subnets to Config Manager at every firewall and security-group layer. See [Firewall Ports](../../firewall-ports/index.mdx).
...
- The device may be on Cumulus pre-5.x, which cannot ZTP. Upgrade and retry.
  ...
  After the boot script starts, test the rendered-configuration path over SFTP
  in the same VRF:

  ```bash
  sudo ip vrf exec mgmt curl --insecure --user ztp:ztp \
    --output /tmp/startup.yaml \
    sftp://<ztp-server>:2222/device/<device-uuid>/startup.yaml
  ```
