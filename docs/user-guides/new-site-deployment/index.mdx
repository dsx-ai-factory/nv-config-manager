...
1. Open the **ZTP logs** panel and switch to the log explorer before filtering by device UUID. Include both the `http-lb` and `sftp` containers: provisioning starts over HTTP/HTTPS, then the default Cumulus and NVOS templates retrieve rendered configuration and certificates over SFTP.
...
| ZTP serves the device | The `http-lb` or `sftp` ZTP container logs contain the Nautobot device UUID | Correlate both containers. An HTTP status diagnoses bootstrap requests; an SFTP path or transfer error diagnoses rendered configuration and certificate retrieval. |
...
`sftp` container handles the default rendered-configuration and certificate
downloads. A complete provisioning trace can cross both containers.
