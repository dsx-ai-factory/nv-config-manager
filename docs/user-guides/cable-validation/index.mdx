...
The report is available in the completed report stage before DCIM status persistence finishes. While persistence is running, the report stage keeps the Markdown table and Excel link visible and displays a progress message; the message is removed when persistence completes. Site workflows collect their children's results and generate the site report before applying cable statuses. Supported providers record `Connected`, `Disconnected`, or `Invalid`; ignored connections are left unchanged. Status updates run concurrently within each device, with a fifteen-minute activity timeout per device. Nautobot also records the validating device workflow ID in a cable note.
