...
The report is available while the report stage is still in progress. During DCIM status persistence, the stage keeps the Markdown table and Excel link visible and displays a progress message; the message is removed and the stage completes once persistence succeeds. Persistence errors leave the report visible with a failure notice and mark the stage failed. Site workflows collect their children's results and generate the site report before applying cable statuses. Supported providers record `Connected`, `Disconnected`, or `Invalid`; ignored connections are left unchanged. Status updates run concurrently within each device, with a fifteen-minute activity timeout per device. Nautobot also records the validating device workflow ID in a cable note when the status changes.
