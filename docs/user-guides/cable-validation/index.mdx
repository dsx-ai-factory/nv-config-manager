...
The Cable Validation workflows validate physical cabling against the intended topology in the configured DCIM. They compare what each switch reports over LLDP (and, where LLDP is unavailable, what its FDB and ARP tables show) against the cabling the DCIM says should be in place, and produce a per-interface report of every mismatch.
...
Site-launched device validations have a ten-minute overall timeout, including retries and waits for manual stage retries. A timed-out device appears among the failed devices in the site report, allowing the remaining results to be reported. Standalone device validations retain their normal stage retry behavior.

The report becomes available before cable statuses are persisted to the DCIM. The report remains visible while its stage is in progress, so operators can begin reviewing results without waiting for the DCIM writes. See [DCIM cable status updates](#dcim-cable-status-updates) for the status mapping and provider behavior.
...
Cable validation runs from the perspective of each in-scope network device. For each enabled interface with a connected interface modeled in the DCIM, it compares the intended far end with observations from the local device in this order:

1. **Does the intended neighbor have the required DCIM data?** The connected device must have a name and role. Missing required data stops validation for the device before any link-state result is produced.
1. **Is the local link operationally up?** If not, the workflow reports `Link is down`. This check applies to both full validation and link-state-only validation after the required DCIM data passes validation.
...
1. **Does LLDP identify the intended neighbor?** The reported neighbor device and interface are compared with the far-end device and interface in the DCIM. The comparison is case-insensitive. The workflow also handles devices that advertise an interface MAC instead of a port or system name, including the supported DPU MAC-offset behavior.
1. **Does the expected far-end MAC appear on the correct local interface?** The workflow takes the MAC from the connected far-end interface in the DCIM and looks for it in the local device's FDB or ARP table.
...
The workflow also reports an LLDP neighbor on an interface that has no intended connection in the DCIM as `Unexpected connection found`. It does not treat every unmodeled FDB entry as an unexpected cable because a port can legitimately learn multiple MAC addresses.
...
## DCIM cable status updates

When the provider supports cable status persistence, validation records these results on each modeled cable:

| Validation result | Persisted status |
| :---------------- | :--------------- |
| Link is down | `Disconnected` |
| Identity mismatch or unsuppressed missing neighbor | `Invalid` |
| Link passes full validation | `Connected` |
| Link is up and passes `cable-validation-link-state-only` validation | `Connected` |
| Interface has `cable-validation-ignore` | No change |
| Missing neighbor is suppressed with **Ignore No Neighbor** | No change |
| Unexpected connection finding | No change |

The configured DCIM integration reads the current value before writing. If the status is already correct, the workflow makes no change. This avoids unnecessary DCIM writes and events on repeated site validations. Deployments without cable-status persistence support treat the update as a no-op.

For a site validation, the `format_result` stage displays the Markdown report and Excel link, then remains **In Progress** while it applies the collected cable statuses. For a standalone device validation, `validate_connections` behaves the same way. Status writes run concurrently within each device and have a fifteen-minute activity timeout per device. The stage becomes **Complete** only after persistence succeeds.

If persistence fails, the stage is marked **Failed**, but its output retains the report and replaces the progress message with a warning that cable statuses may be partially updated. Resolve the reported DCIM error and rerun validation; unchanged statuses will be skipped on the next run.

## Validation tags

Apply validation tags to the interface on the in-scope network device—the interface from which the workflow collects link, LLDP, FDB, and ARP state. For a high-speed leaf-to-host connection, this is normally the leaf switch interface. The cable and connected far-end interface must still be modeled in the DCIM.

### `cable-validation-link-state-only`

Use `cable-validation-link-state-only` when the intended connection is known but there is not yet enough neighbor identity data for a meaningful full check. The common initial bringup case is a high-speed host port whose MAC addresses are not delivered with the original inventory data. Those MACs become available only after the compute management system discovers the host and updates the DCIM. Hosts may also provide no reliable LLDP identity during this stage.
...
1. Confirm the cable and both endpoints are modeled correctly in the DCIM.
...
1. Allow the compute management system to discover the hosts and populate their high-speed interface MAC addresses in the DCIM.
...
The report lists findings, not every passing interface. An up link-state-only interface therefore does not appear with a special “partial validation” result. Before final site acceptance, filter DCIM interfaces by the `cable-validation-link-state-only` tag, resolve every remaining temporary use, and run full validation again.

To add or remove the tag, open the local switch interface in the DCIM, edit its **Tags** field, and save the interface. For many host-facing ports, use the interface list and bulk-edit the selected interfaces. The workflow reads the current tags at the start of each run, so re-run validation after changing them.
...
- **Devices have completed ZTP** and reached the `Provisioned` (or `Active`) status in the DCIM. Running validation against devices that are still provisioning or unreachable produces noisy failures. See [New Site Bringup](../new-site-deployment/index.mdx) for the bringup sequence.
- **Intended cabling is captured in the DCIM.** The workflow compares observed state against enabled interfaces and connected endpoints in the DCIM, so the inventory data must reflect the design. Use `cable-validation-link-state-only` temporarily when an intended connection should be checked for link state but its far-end identity data is not available. Use `cable-validation-ignore` only for interfaces that are deliberately outside validation scope.
...
1. Click the **+** in the top right and select **SiteCableValidationWorkflow**.
   ...
   | **Device Types** | Optional device-type filter. When set, only devices whose DCIM device type IDs are in this list are included. | No |
   ...
   | **Tenant** | DCIM tenant for the run. | Yes |
...
1. When **Generate Cable Validation report** starts, the status page selects it and displays the report while DCIM persistence continues. You can review the Markdown table or download the Excel workbook immediately. The stage remains **In Progress** until the cable status writes finish.
...
| **Tenant** | Optional DCIM tenant filter to narrow the device list. | No |
...
A typical device report is available in well under a minute, dominated by the round-trip time to query LLDP and the FDB. The workflow may remain in progress longer while it persists cable statuses to the DCIM.
...
1. **`get_device_data` — Fetch the device record from the DCIM.**
...
1. **`validate_device_hostname` — Confirm the device's running hostname matches the DCIM.**

   Logs into the device and reads its configured hostname, then compares it against the DCIM name. A mismatch fails the workflow before any cabling work begins, since cabling validation against the wrong device would produce misleading findings.

1. **`get_device_intended_neighbors` — Read intended cabling from the DCIM.**

   Pulls every enabled interface with a connected interface from the DCIM. For each connection, it records the far-end device, interface, role, rack position, and interface MAC when present. It also records `cable-validation-ignore` and `cable-validation-link-state-only` tags on the local interface.
...
1. **`get_device_mac_table` — Read the FDB and ARP table from the device.**
   ...
   Pulls the MAC table (FDB) and ARP table off the device. MAC and ARP entries are the fallback validator for links where LLDP is not available — most commonly IPMI/BMC connections to hosts. When LLDP is absent, the FDB MAC must match the MAC recorded in the DCIM.

1. **`validate_connections` — Compare intended vs actual and produce the report.**

   Joins the three data sources and applies the decision order in [How each connection is evaluated](#how-each-connection-is-evaluated). It validates the required intended-neighbor fields before checking link state. After that prerequisite passes, an up interface marked link-state-only passes immediately; otherwise either LLDP or the expected far-end MAC must match. The stage then classifies every mismatch (link down, wrong neighbor, no neighbor found, unexpected connection), decorates findings with the stable `ID` hash used across runs, and formats the per-device Markdown report. For a standalone device run, the report appears while this stage persists cable statuses; a site-launched child returns its pending updates to the parent instead. This stage is not retryable — if validation logic or DCIM persistence raises, the run fails outright rather than masking a real error behind a retry.
...
The results in the report show the device/port pair being validated (Start Device/Port) and then what the intended connection should be (based on what is in the DCIM) compared to the actual observed connection on the device.
...
The result page includes a download link so you still get the full result set when it is too large to display in the UI.
...
The cabling modeled in the DCIM does not match what was found on the network. This can be due to:

- **LLDP data** — the physical cabling does not match what is modeled in the DCIM.
- **MAC address** — the MAC seen on the switch FDB does not match the expected MAC in the DCIM. Either the inventory MAC is wrong, or the physical cabling is wrong. This is used when LLDP is not available, usually for IPMI/BMC connections.
...
A neighbor was found that is not modeled in the DCIM. This could mean the inventory is out of date, or that there are connections not in scope for Config Manager validation (commonly custom cabling for the site or connections to devices not managed in the DCIM).

If there are connections in the DCIM that are deliberately outside validation scope, add the `cable-validation-ignore` tag to the in-scope switch interfaces and re-run the cable validation workflow. If the connection should be validated later but currently lacks host MAC or reliable LLDP data, use `cable-validation-link-state-only` instead.

## Common issues and troubleshooting

**One or more devices are listed as failed.**

A failed device means the per-device child workflow itself errored — most often because the device is unreachable over its management IP or its hostname does not match the DCIM. The device's findings are excluded from the report; fix the underlying issue and either re-run the site workflow or run a targeted device cable validation against just that device. The results page lists unreachable devices separately and excludes their results from the report.

**Hostname validation failed.**

The hostname configured on the device does not match the DCIM. Either the device was renamed in the inventory without re-applying configuration, or you are running validation against the wrong device. Reconcile the names — typically by re-rendering and re-applying the device's configuration — and re-run.

**Stage `get_device_actual_neighbors` or `get_device_mac_table` failed.**

The device is unreachable, the management credentials are wrong, or the device is on an unsupported platform. Confirm the device is reachable on its management IP, that ZTP has completed, and that the platform field in the DCIM is set correctly. If the device was just reprovisioned, give LLDP a minute or two to populate before retrying.

**The report is visible, but its stage says DCIM cable status persistence failed.**

The validation itself completed, but one or more cable status writes failed. The report remains valid for the observations collected during the run. Some cable statuses may have been updated before the failure. Resolve the DCIM connectivity, permissions, missing-cable, or bootstrap-data error shown by the workflow and rerun validation. The rerun skips statuses that already match.
