...
To find existing tags in Nautobot, expand **Organization** in the left sidebar,
scroll to **Metadata**, and select **Tags**. Search for `dhcp` to show only
DHCP-related tags.
...
<Note>
If an IP tagged with `dhcp-pool` is assigned to an interface that has DHCP
subnet options applied, those options also apply to the DHCP pool subnet. For
details, refer to [DHCP options](#dhcp-options).
</Note>
