...
def test_shared_constructor_ports_and_configured_credentials(name, default_port):
    config = ConfigParser()
    ...
    with (
        ...
        for kwargs, expected_port in [
            ...
            load.reset_mock()
            connection = getattr(legacy, name)("host", **kwargs)
...
def test_service_base_still_requires_a_port():
    ...
    with patch.object(legacy_base, "load_config", return_value=config) as load:
        ...
        assert legacy.NetworkConnection("host", 22)._port == 22
        load.assert_called_once_with()
