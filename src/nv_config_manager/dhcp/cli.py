...
# The client already retries each cancelled request. A cycle-level attempt
# re-reads the whole inventory, so one extra is the useful maximum: it usually
# lands after the replica's replay burst and saves a whole refresh_interval of
# staleness. Once the attempts are spent the cycle is skipped, so the cost of
# giving up early is bounded by refresh_interval.
CANCELLED_READ_ATTEMPTS = 2
CANCELLED_READ_RETRY_DELAY_SECONDS = 5.0
...
async def _refresh_cycle_async(
    ...
    """Run one refresh, retrying while the DCIM's datastore cancels the read.

    Raises DCIMReadCancelledError once the attempts are spent, leaving the
    ...
    while True:
        ...
        try:
            ...
            logger.warning(
                ...
                f"(attempt {attempt}/{CANCELLED_READ_ATTEMPTS}): "
                f"{escape_log_newlines(str(exc))}"
            )
            await asyncio.sleep(CANCELLED_READ_RETRY_DELAY_SECONDS)
    ...
    try:
        async with dcim_client_session(config) as dcim_client:
            while True:
                ...
                # container. A read the DCIM's datastore cancelled is the
                # exception: restarting only re-runs into the same busy
                # replica, and skipping the publish
