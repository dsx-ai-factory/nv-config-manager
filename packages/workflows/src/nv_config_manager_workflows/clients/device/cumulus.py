...
class CumulusConnection(NetworkConnection):
    ...
    def get(
        ...
        try:
            ...
            logger.error("Device HTTP GET timed out for %s", self._host)
        ...
        if rsp.status_code != 200 and raise_on_failure:
            ...
            logger.error(
                "Device HTTP GET failed for %s with status %d",
                self._host,
                rsp.status_code,
            )
    ...
    def post(
        ...
        try:
            ...
            logger.error("Device HTTP POST timed out for %s", self._host)
            raise NetworkDeviceException(msg) from error
    ...
    def patch(
        ...
        try:
            ...
            logger.error("Device HTTP PATCH timed out for %s", self._host)
            raise NetworkDeviceException(msg) from error
    ...
    def delete(self, url: str, params: Any = None, timeout: int = 30) -> requests.Response:
        ...
        try:
            ...
            logger.error("Device HTTP DELETE timed out for %s", self._host)
            raise NetworkDeviceException(msg) from error
