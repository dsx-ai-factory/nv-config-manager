...
class NetworkConnection:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize with credentials resolved by the host service."""
        self._host = host
        self._port = port
        self._username = settings["username"]
        self._passwords_to_try = list(settings["passwords"])
    ...
    def _try_passwords_with_callback(
        ...
        for idx, password in enumerate(passwords, 1):
            try:
                logger.debug(
                    "Attempting authentication to %s (attempt %d/%d)",
                    self._host,
                    idx,
                    len(passwords),
                )
                ...
                logger.debug(
                    "Successfully authenticated to %s (attempt %d)",
                    self._host,
                    idx,
                )
                return result
