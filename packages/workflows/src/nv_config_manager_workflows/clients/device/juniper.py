...
from nv_config_manager_workflows.secret_redaction import redact_junos_secrets
...
def _mac_entry_from_junos(data: dict[str, Any]) -> DeviceMacEntry | None:
    ...
    try:
        ...
        logger.warning("Invalid MAC in Junos MAC table entry, skipping")
        return None
...
def _arp_table_from_junos(entries: list[dict[str, Any]]) -> DeviceArpTable:
    ...
    for entry in entries:
        ...
        if not (ip and mac and interface):
            logger.warning("ARP entry missing data, skipping")
            continue
        try:
            result.add_entry(ip, mac, interface.split(".")[0])
        except (ValueError, netaddr.core.AddrFormatError):
            logger.warning("Invalid IP/MAC in Junos ARP entry, skipping")
            continue
    return result
...
class JuniperConnection(NetworkConnection):
    ...
    def _device_factory(self) -> Any:
        """SDK constructor, overridable by host integrations."""
        return Device

    @property
    def _config_factory(self) -> Any:
        """SDK configuration context, overridable by host integrations."""
        return Config
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(host, port, settings=settings)
    ...
    def _device_timeout(self, seconds: int) -> Iterator[Device]:
        ...
        try:
            ...
            try:
                ...
                logger.debug("Unable to restore RPC timeout on %s", self._host)
    ...
    def close(self) -> None:
        ...
        if device is not None:
            try:
                ...
                logger.debug("Error closing NETCONF session to %s", self._host)
            self._device = None
    ...
    def _discard_candidate(cu: Config, host: str) -> None:
        ...
        try:
            ...
            logger.debug("Failed to discard candidate config on %s", host)
    ...
    def perform_candidate_diff(self, new_configuration: str, partial: bool = False) -> str:
        ...
        try:
            with (
                ...
                self._config_factory(device, mode="exclusive") as cu,
            ):
                try:
    ...
    def commit_candidate_config(
        ...
        *,
        ...
        device = self._get_device()
        try:
            with self._config_factory(device, mode="exclusive") as cu:
                self._load_full_config(cu, new_configuration)
                diff = cu.diff()
    ...
    def save_rescue_configuration(self) -> None:
        ...
        device = self._get_device()
        try:
            self._config_factory(device).rescue(action="save")
        except (RpcError, ConnectError) as error:
            raise NetworkDeviceException(
    ...
    def delete_rescue_configuration(self) -> None:
        ...
        device = self._get_device()
        try:
            self._config_factory(device).rescue(action="delete")
        except (RpcError, ConnectError) as error:
            raise NetworkDeviceException(
