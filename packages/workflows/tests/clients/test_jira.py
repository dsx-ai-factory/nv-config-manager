...
"""Behavior-preservation tests for the reusable Jira provider."""
...
def test_attachment_size_limit_remains_ten_mebibytes() -> None:
...
async def test_request_urls_preserve_unencoded_main_behavior() -> None:
    issue_key = "GNI-1234/child?expand=true#fragment"
    issue_url = f"{BASE_URL}/rest/api/latest/issue/{issue_key}"
    session, response = _session_response(status=200)
    response.json.side_effect = [
        {"self": issue_url, "fields": {}},
        [{"id": "42"}],
        {"id": "43"},
    ]
    provider = _provider()
    ...
    with patch.object(provider, "_ensure_session", new=AsyncMock(return_value=session)):
        ...
        await provider.upload_attachment(issue_key, "diagnostics.txt", b"data", "text/plain")
...
async def test_validate_issue_returns_main_response_shape() -> None:
...
async def test_validate_issue_preserves_missing_issue_error() -> None:
...
async def test_validate_issue_preserves_other_http_error() -> None:
...
async def test_upload_attachment_preserves_success_statuses(status: int) -> None:
...
async def test_upload_attachment_preserves_fallback_identifiers(
...
async def test_upload_attachment_preserves_http_error() -> None:
...
@pytest.mark.parametrize("status", [200, 201])
async def test_add_comment_preserves_success_statuses(status: int) -> None:
...
async def test_add_comment_preserves_http_error() -> None:
